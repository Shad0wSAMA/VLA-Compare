...
def test_dagger_config_defaults():
    ...
    assert cfg.wrist_roll_offset_deg == 0.0


def test_dagger_wrist_roll_offset_wraps_without_mutating_input():
    from lerobot.rollout.strategies.dagger import _apply_wrist_roll_offset

    action = {"wrist_roll.pos": -135.0, "gripper.pos": 10.0}
    offset_action = _apply_wrist_roll_offset(action, -90.0)

    assert offset_action == {"wrist_roll.pos": 135.0, "gripper.pos": 10.0}
    assert action["wrist_roll.pos"] == -135.0


def test_dagger_transition_applies_inverse_offset_to_leader(monkeypatch):
    from lerobot.rollout import DAggerStrategyConfig
    from lerobot.rollout.strategies import DAggerPhase, DAggerStrategy
    from lerobot.rollout.strategies import dagger as dagger_module

    teleop = MagicMock()
    teleop.feedback_features = {"wrist_roll.pos": float}
    ctx = SimpleNamespace(hardware=SimpleNamespace(teleop=teleop, robot_wrapper=MagicMock()))
    smooth_move = MagicMock()
    monkeypatch.setattr(dagger_module, "teleop_smooth_move_to", smooth_move)

    strategy = DAggerStrategy(DAggerStrategyConfig(wrist_roll_offset_deg=-90.0))
    strategy._apply_transition(
        ...
        MagicMock(),
        ctx,
        {"wrist_roll.pos": -135.0, "gripper.pos": 10.0},
    )

    smooth_move.assert_called_once_with(
        teleop, {"wrist_roll.pos": -45.0, "gripper.pos": 10.0}
    )
