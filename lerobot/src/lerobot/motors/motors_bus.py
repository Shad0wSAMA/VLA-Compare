...
class MotorsBusBase(abc.ABC):
    ...
    @abc.abstractmethod
    def disable_torque(self, motors: str | list[str] | None = None, num_retry: int = 3) -> None:
...
class SerialMotorsBus(MotorsBusBase):
    ...
    def record_ranges_of_motion(
        ...
        start_positions = self.sync_read("Present_Position", motor_names, normalize=False, num_retry=3)
        mins = start_positions.copy()
        maxes = start_positions.copy()

        user_pressed_enter = False
        while not user_pressed_enter:
            positions = self.sync_read("Present_Position", motor_names, normalize=False, num_retry=3)
    ...
    def ping(self, motor: NameOrID, num_retry: int = 3, raise_on_error: bool = False) -> int | None:
        """Ping a single motor and return its model number.

        Args:
            motor (NameOrID): Target motor (name or ID).
            num_retry (int, optional): Extra attempts before giving up. Defaults to `3`.
