...
def _apply_wrist_roll_offset(action: dict[str, Any], offset_deg: float) -> dict[str, Any]:
    """Return a copy of ``action`` with a wrapped wrist-roll offset applied."""
    if not offset_deg or "wrist_roll.pos" not in action:
        return action

    offset_action = dict(action)
    wrist_roll = offset_action["wrist_roll.pos"] + offset_deg
    offset_action["wrist_roll.pos"] = (wrist_roll + 180.0) % 360.0 - 180.0
            ...
            try:
                while not events.stop_recording.is_set() and not ctx.runtime.shutdown_event.is_set():
                    ...
                    if phase == DAggerPhase.CORRECTING:
                        ...
                        processed_teleop = _apply_wrist_roll_offset(
                            processed_teleop, self.config.wrist_roll_offset_deg
                        )
    ...
    def _apply_transition(
        ...
        if old_phase == DAggerPhase.AUTONOMOUS and new_phase == DAggerPhase.PAUSED:
            ...
            if teleop_supports_feedback(teleop) and prev_action is not None:
                ...
                # follower = leader + offset, so leader = follower - offset.
                leader_target = _apply_wrist_roll_offset(
                    prev_action, -self.config.wrist_roll_offset_deg
                )
                teleop_smooth_move_to(teleop, leader_target)
            ...
            if not teleop_supports_feedback(teleop) and prev_action is not None:
                ...
                processed = _apply_wrist_roll_offset(processed, self.config.wrist_roll_offset_deg)
