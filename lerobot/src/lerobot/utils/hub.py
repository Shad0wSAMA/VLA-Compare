...
def normalize_hub_repo_id(repo_id: str | Path) -> str:
    """Return a Hub repo ID with POSIX separators on every platform.

    Policy identifiers can pass through a ``Path``-typed config field. On
    Windows, that turns ``owner/model`` into ``owner\\model``, which is a valid
    local path spelling but not a valid Hugging Face Hub repository ID.
    Call this only after determining that the value is not an existing local
    directory.
    """
    return str(repo_id).replace("\\", "/")
