...
def update_last_checkpoint(checkpoint_dir: Path) -> Path:
    ...
    elif os.name == "nt" and last_checkpoint_dir.exists():
        # A Windows directory junction is not reported as ``is_symlink()``.
        # ``rmdir`` removes the junction itself, not the checkpoint it targets.
        subprocess.run(
            ["cmd", "/c", "rmdir", "/s", "/q", str(last_checkpoint_dir)],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    try:
        ...
        # A directory junction normally works for standard user accounts.
